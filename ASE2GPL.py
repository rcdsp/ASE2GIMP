#!/usr/bin/env python
# Author: Chris Mohler, Roy Curtis, Ricardo Cordero
# Copyright 2008 Chris Mohler
# License: GPL v3
# Portions of this code were taken from easyrgb.com
# Python script to convert ase palettes into gpl palettes without gimp
# Updated for later ASE format by Roy Curtis
# Updated for command line usage by Ricardo Cordero

# Use this on command line if you want to import .ase palettes in any software that only supports importing .gpl palettes
# and you don't use or want to install gimp for conversion.

# To use this in command line just pass the path to a directory where you have your .ase palettes. The script will create
# a gpl directory (if does not exist already) inside yout palettes directory and will output the converted palettes there.
...
out_dir = os.path.join(palettes_dir, 'gpl')


def ase_converter(dir):
    # Run the converte for each .ase file in the directory specified
    for filename in os.listdir(dir):
        if filename.endswith(".ase"):
            print("ASE File: " + filename)

            try:
                # Needs to be in read-binary mode, else Windows will break reading.
                # Auto-closes file on finish or exception because of "with"
                with open(os.path.join(dir, filename), 'rb') as ase_file:
                    do_convert(ase_file)
                ...
                print('Error: %s\n' %
                      traceback.format_exc() + "Something happened")
...
def do_convert(ase_file):
    ...
    if ase_version_major != 1:
        raise Exception(
            "Major version of given file is not 1; not compatiable with script.")

    if ase_version_minor != 0:
        print("Warning: Minor version of given file is not 0; might not work with this script.")
    ...
    for block in range(ase_nbblocks):
        ...
        block_data = io.StringIO(ase_file.read(block_len))
        ...
        if block_type == PAL_START:
            ...
            pal_title = str.title(
                str(read_ase_string(block_data))).replace(" ", "")
        ...
        ###########################
            ...
            if pal_gpl == "":
                raise Exception(
                    "Unexpected palette entry before palette start")
            ...
            if col_model == "LAB " or col_model == "CMYK":
                if not inaccuracy_warning:
                    print(
                        "Warning: Converting from LAB or CMYK colors is inaccurate and may result in slightly-off RGB values")
                    inaccuracy_warning = True

            if col_model == "RGB ":
                # read 4 bytes, unpack to float - little endian
                red = unpack_from('>f',  block_data.read(4))
                ...
                # read 4 bytes, unpack to float - little endian
                lab_L = unpack_from('>f',  block_data.read(4))
                ...
                # X from 0 to  95.047 (Observer = 2 deg, Illuminant = D65)
                var_X = X / 100
                ...
                # read 4 bytes, unpack to float - little endian
                cmyk_C = unpack_from('>f',  block_data.read(4))
                ...
                print("Warning: Unknown color model \"" +
                      col_model + "\", skipped")
        ...
        ###############################
            ...
            if pal_ncols == 0:
                print(
                    "Warning: Could not import any colors from palette \"" + pal_title + "\"")
            else:
                # Target GPL path and file name
                pal_path = os.path.join(out_dir,  (pal_title + ".gpl"))
                ...
                if os.path.isfile(pal_path):
                    print("Warning: Palette \"" + pal_path +
